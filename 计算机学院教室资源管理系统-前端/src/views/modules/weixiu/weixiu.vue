<template>
  <div class="app-container">
    <div v-if="$store.state.user.panduan=='后勤'">
      <div class="mg-b">
      ...
      <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="状态" width="80"
         :filters="[{ text: '待处理', value: '0' }, { text: '处理完成', value: '1' }, { text: '处理中', value: '2' }]"
        ...
        <el-table-column prop="name" label="进度">
          <template slot-scope="scope">
            <span style="float:left" v-if="scope.row.feedProgress" class="" :title="scope.row.feedProgress">{{ scope.row.feedProgress.replace(/;/g, '->') }}
            </span>
            <span style="float:left" v-if="scope.row.repairState==1">完成</span>
            <span style="float:left" v-if="scope.row.repairState!=1">...</span>
            <span v-if="!scope.row.feedProgress">-</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="教学楼" width="80">
          <template slot-scope="scope">
            <span class="shenglue" :title="scope.row.teachingBuildingName">{{ scope.row.teachingBuildingName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="date" label="教室" width="80">
          <template slot-scope="scope">
            <span class="shenglue" :title="scope.row.roomName">{{ scope.row.roomName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="date" label="设施" width="100"
        :filters="sheshis"
        :filter-method="filterSheshi">
          <template slot-scope="scope">
    ...
    <div v-else>
      ...
      <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="进度">
          <template slot-scope="scope">
            <span v-if="scope.row.feedProgress" class="" :title="scope.row.feedProgress">{{ scope.row.feedProgress.replace(/;/g, '->') }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="教学楼" width="80">
    ...
    <el-dialog title="报修" width="50%" :visible.sync="dialog">
      <el-row class="el-row">
        <el-row class="el-row">
          <el-col :span="span1">
...
export default {
  ...
  methods: {
    filterSheshi (value, row) {
      return row.materielType === value
    },
    filterTag (value, row) {
      return row.repairState + '' === value
    },
